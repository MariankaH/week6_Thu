...
```{r}
Female_fall_admissions <- unintentional_injuries %>% 
...
```{r}
falls_admissions_75_summary <- Female_fall_admissions %>% 
  group_by(HBR) %>% 
  summarise(admissions = sum(NumberOfAdmissions))
```
...
joined_data <- NHS_healthboards %>% 
  full_join(falls_admissions_75_summary, by = c("HBCode" = "HBR"))
```
...
```{r}
fall_admissions_map <- joined_data %>% 
  ggplot(aes(fill = admissions)) +
  geom_sf() +
  scale_fill_distiller(palette = 10, direction = 1) + 
  labs(title = "Map Displaying Number of Female Fall Admissions",
       subtitle = "These women were all 75+ years and fell at home.") +
  theme_bw()
```
